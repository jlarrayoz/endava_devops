...
#Se remplaza palabra en el archivo textoEjemplo.txt (del dir resoruces) y se guarda el resultado en textoRemplazoPS.txt
#Se hace esto para no modificar el archivo origen.
(Get-Content ../resources/textoEjemplo.txt).replace($($args[0]), $($args[1])) | Set-Content ../resources/textoRemplazoPS.txt